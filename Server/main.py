...
def mfByNoc(noc):
    res_m = engine.execute("SELECT count(*) FROM athlete_events WHERE NOC = '" + noc + "' AND Sex = 'M'")
    res_f = engine.execute("SELECT count(*) FROM athlete_events WHERE NOC = '" + noc + "' AND Sex = 'F'")
    res_m = [(row[0]) for row in res_m][0]
    res_f = [(row[0]) for row in res_f][0]
    res = [{'values':[res_m,res_f],'labels':['Male','Female'], 'type':'pie'}]
    return j.dumps(res)
